...
export const wordpressAPI = {
  ...
  async getRestaurantCategories(): Promise<Category[]> {
    try {
      ...
      return categories.filter(
        (cat: Category & { count: number }) => cat.count > 0,
      );
    } catch (error) {
  ...
  async getAccommodationCategories(): Promise<Category[]> {
    try {
      ...
      if (!response.ok)
        throw new Error("Failed to fetch accommodation categories");
      const categories = await response.json();
      return categories.filter(
        (cat: Category & { count: number }) => cat.count > 0,
      );
  ...
  async getCategoryCounts(): Promise<CategoryCount> {
    try {
      const [restaurants, businesses, accommodations, places] =
        await Promise.all([
      ...
      return {
        ...
        accommodations: parseInt(
          accommodations.headers.get("X-WP-Total") || "0",
        ),
  ...
  async searchListings(query: string, limit = 10): Promise<Listing[]> {
    try {
      const [restaurants, businesses, accommodations, places] =
        await Promise.all([
          fetchWp(
            `${WP_API_BASE}/restaurant?search=${encodeURIComponent(query)}&per_page=${limit}`,
            { next: { revalidate: 3600 } },
          ),
          fetchWp(
            `${WP_API_BASE}/business?search=${encodeURIComponent(query)}&per_page=${limit}`,
            { next: { revalidate: 3600 } },
          ),
          fetchWp(
            `${WP_API_BASE}/accommodation?search=${encodeURIComponent(query)}&per_page=${limit}`,
            { next: { revalidate: 3600 } },
          ),
          fetchWp(
            `${WP_API_BASE}/places?search=${encodeURIComponent(query)}&per_page=${limit}`,
            { next: { revalidate: 3600 } },
          ),
        ]);
      ...
      return [...results[0], ...results[1], ...results[2], ...results[3]].slice(
        0,
        limit,
      );
    } catch (error) {
  ...
  async getListingBySlug(type: string, slug: string): Promise<Listing | null> {
    try {
      const response = await fetchWp(
        `${WP_API_BASE}/${type}?slug=${slug}&_embed`,
        { next: { revalidate: 3600 } },
      );
  ...
  async getBlogPostBySlug(slug: string): Promise<BlogPost | null> {
    try {
      const response = await fetchWp(
        `${WP_POSTS_API}/posts?slug=${slug}&_embed`,
        { next: { revalidate: 3600 } },
      );
      if (!response.ok) throw new Error("Failed to fetch blog post");
  ...
  async getPageBySlug(slug: string): Promise<Page | null> {
    try {
      const response = await fetchWp(
        `${WP_POSTS_API}/pages?slug=${slug}&_embed`,
        { next: { revalidate: 3600 } },
      );
      if (!response.ok) throw new Error("Failed to fetch page");
