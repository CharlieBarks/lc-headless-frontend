...
const DEFAULT_IMAGES = {
  ...
  place: 'https://images.pexels.com/photos/2739664/pexels-photo-2739664.jpeg?auto=compress&cs=tinysrgb&w=800',
...
export const wordpressAPI = {
  ...
  async getRestaurantCategories(): Promise<Category[]> {
    try {
      const response = await fetch(`${WP_API_BASE}/restaurant/categories?per_page=100`, { cache: 'no-store' });
  ...
  async getBusinessCategories(): Promise<Category[]> {
    try {
      const response = await fetch(`${WP_API_BASE}/business/categories?per_page=100`, { cache: 'no-store' });
  ...
  async getAccommodationCategories(): Promise<Category[]> {
    try {
      const response = await fetch(`${WP_API_BASE}/accommodation/categories?per_page=100`, { cache: 'no-store' });
  ...
  async getPlaceCategories(): Promise<Category[]> {
    try {
      const response = await fetch(`${WP_API_BASE}/places/categories?per_page=100`, { cache: 'no-store' });
  ...
  async getBlogPosts(limit = 3): Promise<BlogPost[]> {
    try {
      const response = await fetch(`${WP_POSTS_API}/posts?per_page=${limit}&_embed`, { cache: 'no-store' });
  ...
  async getCategoryCounts(): Promise<CategoryCount> {
    try {
      const [restaurants, businesses, accommodations, places] = await Promise.all([
        fetch(`${WP_API_BASE}/restaurant?per_page=1`, { cache: 'no-store' }),
        fetch(`${WP_API_BASE}/business?per_page=1`, { cache: 'no-store' }),
        fetch(`${WP_API_BASE}/accommodation?per_page=1`, { cache: 'no-store' }),
        fetch(`${WP_API_BASE}/places?per_page=1`, { cache: 'no-store' })
      ]);
  ...
  async searchListings(query: string, limit = 10): Promise<Listing[]> {
    try {
      const [restaurants, businesses, accommodations, places] = await Promise.all([
        fetch(`${WP_API_BASE}/restaurant?search=${encodeURIComponent(query)}&per_page=${limit}`, { cache: 'no-store' }),
        fetch(`${WP_API_BASE}/business?search=${encodeURIComponent(query)}&per_page=${limit}`, { cache: 'no-store' }),
        fetch(`${WP_API_BASE}/accommodation?search=${encodeURIComponent(query)}&per_page=${limit}`, { cache: 'no-store' }),
        fetch(`${WP_API_BASE}/places?search=${encodeURIComponent(query)}&per_page=${limit}`, { cache: 'no-store' })
      ]);
  ...
  async getListingById(type: string, id: string): Promise<Listing | null> {
    try {
      const response = await fetch(`${WP_API_BASE}/${type}/${id}?_embed`, { cache: 'no-store' });
  ...
  async getBlogPostById(id: string): Promise<BlogPost | null> {
    try {
      const response = await fetch(`${WP_POSTS_API}/posts/${id}?_embed`, { cache: 'no-store' });
  ...
  async getBlogPostBySlug(slug: string): Promise<BlogPost | null> {
    try {
      const response = await fetch(`${WP_POSTS_API}/posts?slug=${slug}&_embed`, { cache: 'no-store' });
      if (!response.ok) throw new Error('Failed to fetch blog post');
  ...
  async getPageById(id: string): Promise<Page | null> {
    try {
      const response = await fetch(`${WP_POSTS_API}/pages/${id}?_embed`, { cache: 'no-store' });
  ...
  async getPageBySlug(slug: string): Promise<Page | null> {
    try {
      const response = await fetch(`${WP_POSTS_API}/pages?slug=${slug}&_embed`, { cache: 'no-store' });
      if (!response.ok) throw new Error('Failed to fetch page');
